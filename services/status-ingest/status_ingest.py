...
EPOCH_PATTERN = re.compile(r"^[0-9]{10,11}$")
...
class Config:
    ...
    def from_env(cls) -> "Config":
        timestamp_tolerance = env_int(
            "STATUS_TIMESTAMP_TOLERANCE_SECONDS",
            300,
            minimum=30,
            maximum=3600,
        )
        nonce_retention = env_int(
            "STATUS_NONCE_RETENTION_SECONDS",
            900,
            minimum=300,
            maximum=86400,
        )
        minimum_nonce_retention = timestamp_tolerance * 2
        if nonce_retention < minimum_nonce_retention:
            raise ConfigurationError(
                "STATUS_NONCE_RETENTION_SECONDSは"
                "STATUS_TIMESTAMP_TOLERANCE_SECONDSの2倍以上が必要です"
            )
        ...
        return cls(
            ...
            max_observation_age_seconds=env_int(
                "STATUS_MAX_OBSERVATION_AGE_SECONDS",
                600,
                minimum=30,
                maximum=86400,
            ),
            ...
            nonce_retention_seconds=nonce_retention,
            observation_retention_days=env_int(
                "STATUS_OBSERVATION_RETENTION_DAYS",
                30,
                minimum=1,
                maximum=365,
            ),
    ...
    def latest(self) -> sqlite3.Row | None:
        with self.connect() as connection:
            return connection.execute(
                ...
                """,
                (self.config.allowed_service_id, self.config.allowed_source),
            ).fetchone()
...
class StatusHandler(BaseHTTPRequestHandler):
    ...
    def _handle_request(
        self,
        callback: Callable[[str], HTTPStatus],
    ) -> None:
    ...
    def _verify_signature(self, body: bytes) -> tuple[int, str]:
        ...
        if (
            version != "v1"
            or EPOCH_PATTERN.fullmatch(timestamp_text) is None
        ...
        timestamp_epoch = int(timestamp_text)
        if abs(time.time() - timestamp_epoch) > self.server.config.timestamp_tolerance_seconds:
            raise RequestError(
                HTTPStatus.UNAUTHORIZED,
                "invalid_signature",
                "署名を検証できません",
    ...
    def _send_common_headers(self, *, public: bool) -> None:
        ...
        if public and self.server.config.cors_origin:
            self.send_header(
                ...
                self.server.config.cors_origin,
            )
...
def validate_payload(
    ...
    *,
    ...
    sent_at = parse_rfc3339(payload["sent_at"], "sent_at")
    now = utc_now()
    ...
    if observation_age < -config.future_observation_tolerance_seconds:
        raise RequestError(
            HTTPStatus.UNPROCESSABLE_ENTITY,
            "future_observation",
            "観測時刻が未来です",
        )
    if abs(sent_at.timestamp() - signature_timestamp_epoch) > config.timestamp_tolerance_seconds:
        raise RequestError(
            HTTPStatus.UNPROCESSABLE_ENTITY,
            "invalid_payload",
            "sent_atと署名Timestampの差が大きすぎます",
        )
