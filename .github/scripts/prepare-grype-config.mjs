...
function parseDate(date, fieldName) {
  ...
  assert(
    ...
    `${fieldName}はYYYY-MM-DD形式で指定してください: ${date}`,
  );
...
function validatePackage(value, index) {
  ...
  assert(
    ...
    `allowlist[${index}].packageはobjectで指定してください`,
  );
  assert(
    typeof value.name === 'string' && value.name.trim().length > 0,
    `allowlist[${index}].package.nameは必須です`,
  );
  ...
  if (value.type !== undefined) {
    assert(
      typeof value.type === 'string' && PACKAGE_TYPES.has(value.type),
      `allowlist[${index}].package.typeが未対応です: ${value.type}`,
    );
  }
...
export function validateAllowlist(input, now = new Date()) {
  ...
  return input.map((entry, index) => {
    assert(
      ...
      `allowlist[${index}]はobjectで指定してください`,
    );
    ...
    assert(
      typeof entry.id === 'string' && VULNERABILITY_ID_PATTERN.test(entry.id),
      `allowlist[${index}].idはCVEまたはGHSA形式で指定してください`,
    );
    assert(
      typeof entry.reason === 'string' && entry.reason.trim().length >= 10,
      `allowlist[${index}].reasonは10文字以上で具体的に記載してください`,
    );
    assert(
      typeof entry.issue === 'string' && ISSUE_URL_PATTERN.test(entry.issue),
      `allowlist[${index}].issueはivRooom/HertaのIssue URLを指定してください`,
    );
    ...
    assert(
      expires <= maxExpiry,
      `allowlist[${index}].expiresは本日から${MAX_EXCEPTION_DAYS}日以内にしてください`,
    );
...
export async function main(args = process.argv.slice(2)) {
  ...
  assert(
    ...
    'Usage: node prepare-grype-config.mjs <allowlist.json> <output.yaml>',
  );
