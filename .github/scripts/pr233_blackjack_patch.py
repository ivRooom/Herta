...
manifest_path = Path('packages/plugin-catalog/src/manifests/mini-games.ts')
manifest = manifest_path.read_text()
manifest = manifest.replace("version: '3.3.0'", "version: '3.4.0'", 1)

config_anchor = "      blackjackDealerHitsSoft17: {"
if config_anchor not in manifest:
    raise SystemExit('blackjack config anchor not found')
config_insert = """      blackjackAnimation: {
        type: 'boolean',
...
"""
manifest = manifest.replace(config_anchor, config_insert + config_anchor, 1)

command_anchor = "    { name: 'blackjack', description: 'Dealerと1対1でBlackjackを開始します' },"
if command_anchor not in manifest:
    raise SystemExit('blackjack command anchor not found')
...
interaction_import = "  type Interaction,\n} from 'discord.js';"
if interaction_import not in text:
    raise SystemExit('discord Interaction import anchor not found')
text = text.replace(interaction_import, "  type Interaction,\n  type User,\n} from 'discord.js';", 1)

metrics_import = "import { blackjackSettlementMetrics } from './mini-games-blackjack-metrics.js';"
if metrics_import not in text:
    raise SystemExit('blackjack metrics import anchor not found')
text = text.replace(
    metrics_import,
...
config_line = "  blackjackDealerHitsSoft17: boolean;"
if config_line not in text:
    raise SystemExit('MiniGamesConfig Blackjack anchor not found')
text = text.replace(
    config_line,
...
session_pattern = re.compile(
    r"interface BlackjackSession extends GameSessionBase \{.*?\n\}\n\ntype GameSession = HighLowSession \| BlackjackSession;",
    re.S,
)
session_replacement = """interface BlackjackDealerSession extends GameSessionBase {
  type: 'blackjack';
  mode: 'dealer';
  deck: PlayingCard[];
  player: PlayingCard[];
...
if count != 1:
    raise SystemExit('Blackjack session block not found')

normalize_anchor = "    blackjackDealerHitsSoft17: source.blackjackDealerHitsSoft17 === true,"
if normalize_anchor not in text:
    raise SystemExit('normalize Blackjack anchor not found')
text = text.replace(
    normalize_anchor,
    normalize_anchor
    + "\n    blackjackAnimation:\n      source.blackjackAnimation === undefined ? true : source.blackjackAnimation === true,"
    + "\n    blackjackAnimationDelayMs: clamp(\n      toInteger(source.blackjackAnimationDelayMs, 450),\n      250,\n      1_500,\n    ),",
    1,
)

execute_pattern = re.compile(r"async function executeBlackjack\(.*?\n\}\n\nasync function handleGameButton", re.S)
execute_replacement = r"""async function executeBlackjack(
...
text, count = execute_pattern.subn(execute_replacement, text, count=1)
if count != 1:
    raise SystemExit('executeBlackjack block not found')

ownership_pattern = re.compile(
    r"  if \(interaction\.guildId !== session\.guildId \|\| interaction\.user\.id !== session\.userId\) \{\n"
    r"    await replyEphemeral\(interaction, 'このゲームは開始した本人だけ操作できます。'\);\n"
    r"    return;\n"
    r"  \}"
)
ownership_replacement = """  if (interaction.guildId !== session.guildId || !canOperateGameSession(session, interaction.user.id)) {
...
if count != 1:
    raise SystemExit('game session ownership block not found')

handler_pattern = re.compile(r"async function handleBlackjackButton\(.*?\n\}\n\nasync function executeGameStats", re.S)
handler_replacement = r"""async function handleBlackjackButton(
  context: MiniGamesRuntimeContext,
  ...
  if (action === 'hit') {
    ...
    await finishButtonMessage(
      interaction,
      animated,
      renderBlackjack(session, false),
...
if count != 1:
    raise SystemExit('handleBlackjackButton block not found')
...
text = text.replace('function playDealer(session: BlackjackSession): void {', 'function playDealer(session: BlackjackDealerSession): void {', 1)
text = text.replace('function renderBlackjack(session: BlackjackSession, revealDealer: boolean): string {', 'function renderBlackjack(session: BlackjackDealerSession, revealDealer: boolean): string {', 1)
text = text.replace('function renderBlackjackFinal(session: BlackjackSession): string {', 'function renderBlackjackFinal(session: BlackjackDealerSession): string {', 1)

insert_marker = 'function buildHighLowRow(sessionId: string): ActionRowBuilder<ButtonBuilder> {'
if insert_marker not in text:
    raise SystemExit('Blackjack helper insertion marker not found')
helpers = r"""function renderBlackjackDealStage(session: BlackjackDealerSession): string {
  return [
    '🃏 **Blackjack — Dealing**',
