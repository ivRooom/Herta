...
const DEICTIC_FILE_CONTENT_REQUEST_PATTERN =
  /(?:\b(?:summari[sz]e|analy[sz]e|read|inspect|describe)\b.{0,30}\b(?:this|that)\s+(?:file|document|report)\b|\bwhat\s+(?:does|is|are)\s+(?:in\s+)?(?:this|that)\s+(?:file|document|report)\b|(?:この|その)(?:ファイル|文書|資料|レポート).{0,30}(?:内容|要約|まとめ|読ん|解析|分析|説明|何が|何を|教えて))/i;
