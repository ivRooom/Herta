...
interface TextMessage { id: string; edit(options: SuggestionMessage): Promise<unknown>; }
...
interface ReplyOptions { content: string; flags?: number; allowedMentions: { parse: [] }; }
...
export const suggestionPlugin = definePlugin<SuggestionConfig, SuggestionClient, PrismaClient>({
  ...
  provideCommands(context) {
    const command: CommandHandler<CommandInteraction> = {
      ...
      async execute(interaction) { await executeSuggestionCommand(context, interaction); },
    };
    return [command];
  },
  provideEvents() {
    return [{
      ...
      async handler(context, ...args) {
        await handleSuggestionComponent(context as SuggestionContext, args[0] as ComponentInteraction | undefined);
      },
...
export function normalizeSuggestionConfig(value: unknown): SuggestionConfig {
  ...
  return {
    ...
    suggestionChannelId: typeof source.suggestionChannelId === 'string' && /^\d+$/.test(source.suggestionChannelId) ? source.suggestionChannelId : null,
    ...
    staffRoleIds: Array.isArray(source.staffRoleIds)
      ...
      : [],
    notifyAuthorOnStatusChange: source.notifyAuthorOnStatusChange === undefined ? true : source.notifyAuthorOnStatusChange === true,
  };
}
...
export function buildSuggestionMessage(snapshot: SuggestionSnapshot): SuggestionMessage {
  ...
  const canVote = snapshot.votingEnabled && (snapshot.status === 'pending' || snapshot.status === 'reviewing');
  const components = canVote ? [{
    type: 1,
    components: [
      { type: 2, style: 2, custom_id: `${CUSTOM_ID_PREFIX}vote:${snapshot.id}:up`, label: `👍 ${snapshot.upvotes}` },
      { type: 2, style: 2, custom_id: `${CUSTOM_ID_PREFIX}vote:${snapshot.id}:down`, label: `👎 ${snapshot.downvotes}` },
    ],
  }] : [];
  return {
...
export function canViewSuggestion(snapshot: SuggestionSnapshot, viewer: { userId: string; canManage: boolean }): boolean {
...
export function formatSuggestionInfo(snapshot: SuggestionSnapshot, viewerUserId: string): string {
  const author = snapshot.anonymous ? '匿名' : snapshot.authorId === viewerUserId ? 'あなた' : `<@${snapshot.authorId}>`;
  const lines = [
...
async function executeSuggestionCommand(context: SuggestionContext, interaction: CommandInteraction): Promise<void> {
  if (!interaction.guildId) { await reply(interaction, 'このコマンドはDiscordサーバー内でのみ利用できます。'); return; }
  const config = normalizeSuggestionConfig(context.config);
  if (!config.enabled) { await reply(interaction, 'Suggestion Pluginは現在無効です。'); return; }
...
async function handleCreate(context: SuggestionContext, config: SuggestionConfig, interaction: CommandInteraction): Promise<void> {
  const content = interaction.options.getString('content', true)?.trim() ?? '';
  if (!content || content.length > MAX_CONTENT_LENGTH) { await reply(interaction, 'contentは1〜1000文字で入力してください。'); return; }
  const channel = await resolveSuggestionChannel(context, config, interaction);
  if (!channel) { await reply(interaction, 'Suggestion投稿先チャンネルを利用できません。Studio設定とBot権限を確認してください。'); return; }
  const id = await createSuggestion(context.prisma, {
    guildId: interaction.guildId!, authorId: interaction.user.id,
    channelId: config.suggestionChannelId ?? interaction.channelId!, content,
    anonymous: config.anonymousSubmissions, votingEnabled: config.enableVoting,
    maxOpenPerUser: config.maxOpenPerUser,
  });
  if (!id) { await reply(interaction, `未処理Suggestionが上限（${config.maxOpenPerUser}件）に達しています。`); return; }
  let published = false;
  try {
...
async function handleInfo(context: SuggestionContext, config: SuggestionConfig, interaction: CommandInteraction): Promise<void> {
  const id = interaction.options.getString('id', true)?.trim() ?? '';
  if (!UUID_PATTERN.test(id)) { await reply(interaction, 'Suggestion IDが正しくありません。'); return; }
  const snapshot = await getSuggestionSnapshot(context.prisma, id, interaction.guildId!);
  ...
  if (!snapshot || !canViewSuggestion(snapshot, { userId: interaction.user.id, canManage })) {
    await reply(interaction, 'Suggestionが見つからないか、表示権限がありません。'); return;
  }
  await reply(interaction, formatSuggestionInfo(snapshot, interaction.user.id));
}

async function handleWithdraw(context: SuggestionContext, interaction: CommandInteraction): Promise<void> {
  const id = interaction.options.getString('id', true)?.trim() ?? '';
  if (!UUID_PATTERN.test(id)) { await reply(interaction, 'Suggestion IDが正しくありません。'); return; }
  const result = await withdrawSuggestion(context.prisma, { id, guildId: interaction.guildId!, authorId: interaction.user.id });
  if (result.outcome === 'not_found_or_forbidden') {
    await reply(interaction, 'Suggestionが見つからないか、取下げ権限がありません。'); return;
  }
  if (result.outcome === 'not_withdrawable') {
    await reply(interaction, 'このSuggestionは現在の状態では取り下げできません。'); return;
  }
  if (result.outcome === 'already_withdrawn') {
    await reply(interaction, `Suggestion \`${id}\` はすでに取り下げ済みです。`); return;
  }
  if (result.snapshot) {
    await updateStoredMessage(context, result.snapshot).catch((error) =>
      context.logger.warn({ err: error, suggestionId: id }, '取下げ後のSuggestionメッセージ更新に失敗しました'),
    );
  }
  await reply(interaction, `Suggestion \`${id}\` を取り下げました。`);
}

async function handleStatus(context: SuggestionContext, config: SuggestionConfig, interaction: CommandInteraction): Promise<void> {
  if (!canManageSuggestion(interaction, config)) {
    await reply(interaction, 'Suggestionの状態変更にはManage Server権限または設定済みStaff Roleが必要です。'); return;
  }
  const id = interaction.options.getString('id', true)?.trim() ?? '';
  const status = interaction.options.getString('status', true) as ManagedSuggestionStatus | null;
  const note = interaction.options.getString('note')?.trim() || null;
  if (!UUID_PATTERN.test(id) || !isManagedSuggestionStatus(status)) { await reply(interaction, 'Suggestion IDまたはstatusが正しくありません。'); return; }
  if (note && note.length > MAX_NOTE_LENGTH) { await reply(interaction, 'Staffコメントは300文字以内で入力してください。'); return; }
  const snapshot = await updateSuggestionStatus(context.prisma, { id, guildId: interaction.guildId!, status, staffNote: note });
  if (!snapshot) { await reply(interaction, 'Suggestionが見つからないか、すでに取り下げ済みです。'); return; }
  await updateStoredMessage(context, snapshot).catch((error) => context.logger.warn({ err: error, suggestionId: id }, 'Suggestionメッセージ更新に失敗しました'));
  if (config.notifyAuthorOnStatusChange) {
    await context.client.users.fetch(snapshot.authorId).then((user) => user.send({
...
async function handleSuggestionComponent(context: SuggestionContext, interaction: ComponentInteraction | undefined): Promise<void> {
  ...
  if (!config.enabled || !config.enableVoting) {
    await interaction.reply({ content: 'Suggestion投票は現在無効です。', flags: EPHEMERAL_FLAG, allowedMentions: { parse: [] } }); return;
  }
  const parsed = parseVoteCustomId(interaction.customId);
  if (!parsed) return;
  const snapshot = await voteSuggestion(context.prisma, { id: parsed.id, guildId: interaction.guildId, userId: interaction.user.id, value: parsed.value });
  if (!snapshot) {
    await interaction.reply({ content: 'このSuggestionには投票できません。', flags: EPHEMERAL_FLAG, allowedMentions: { parse: [] } }); return;
  }
  await interaction.update(buildSuggestionMessage(snapshot));
}

async function resolveSuggestionChannel(context: SuggestionContext, config: SuggestionConfig, interaction: CommandInteraction): Promise<TextChannel | null> {
...
export function formatSuggestionListPages(records: readonly SuggestionListRecord[]): string[] {
  ...
  for (const record of records) {
    ...
    const next = `${current}\n\n${line}`;
    if (next.length > MAX_LIST_PAGE_LENGTH) { pages.push(current); current = `**あなたのSuggestion一覧（続き）**\n\n${line}`; }
    else current = next;
  }
  pages.push(current);
  return pages;
...
function toInteger(value: unknown, fallback: number): number { const parsed = Number(value); return Number.isInteger(parsed) ? parsed : fallback; }
