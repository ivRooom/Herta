...
const MAX_MOONSHOT_API_KEY_CHARS = 4096;

type MoonshotCredentialBody = { apiKey: string };

export async function GET() {
  const userId = await authorizedAdminUserId();
  if ('response' in userId) return userId.response;

  const status = await getRuntimeSecretStatus(prisma, MOONSHOT_API_KEY_RUNTIME_SECRET);
  return noStoreJson({
    provider: 'moonshot',
    configured: status.configured,
    environmentFallbackConfigured: hasMoonshotEnvironmentFallback(),
...
export async function PUT(request: Request) {
  ...
  try {
    const status = await setRuntimeSecret(prisma, {
      name: MOONSHOT_API_KEY_RUNTIME_SECRET,
...
export async function DELETE(request: Request) {
  ...
  await deleteRuntimeSecret(prisma, MOONSHOT_API_KEY_RUNTIME_SECRET);
  return noStoreJson({
    provider: 'moonshot',
    configured: false,
    environmentFallbackConfigured: hasMoonshotEnvironmentFallback(),
...
async function parseCredentialBody(
  request: Request,
): Promise<{ value: MoonshotCredentialBody } | { response: NextResponse }> {
  try {
    const bytes = await readRequestBodyBytes(request, MAX_REQUEST_BODY_BYTES);
    const value = JSON.parse(Buffer.from(bytes).toString('utf8')) as unknown;
    if (!isCredentialBody(value)) {
      return { response: noStoreJson({ error: 'Moonshot APIキーの入力が不正です' }, 400) };
    }
    return { value };
  } catch (error) {
...
function isCredentialBody(value: unknown): value is MoonshotCredentialBody {
  if (!isRecord(value) || Object.keys(value).some((key) => key !== 'apiKey')) return false;
  if (typeof value.apiKey !== 'string') return false;
  const apiKey = value.apiKey.trim();
  return (
    apiKey.length > 0 && apiKey.length <= MAX_MOONSHOT_API_KEY_CHARS && !/[ \r\n]/.test(apiKey)
  );
}

function hasMoonshotEnvironmentFallback(): boolean {
...
function runtimeSecretFailureResponse(error: unknown): NextResponse {
  if (error instanceof RuntimeSecretError) {
    ...
    if (error.code === 'invalid_secret_name' || error.code === 'invalid_secret_value') {
      return noStoreJson({ error: 'Moonshot APIキーの入力が不正です' }, 400);
    }
  }
  return noStoreJson({ error: 'Moonshot APIキーを保存できませんでした' }, 500);
}
