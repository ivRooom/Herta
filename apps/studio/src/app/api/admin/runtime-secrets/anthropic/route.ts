...
const MAX_ANTHROPIC_API_KEY_CHARS = 4096;

type AnthropicCredentialBody = { apiKey: string };

export async function GET() {
  const userId = await authorizedAdminUserId();
  if ('response' in userId) return userId.response;

  const status = await getRuntimeSecretStatus(prisma, ANTHROPIC_API_KEY_RUNTIME_SECRET);
  return noStoreJson({
    provider: 'anthropic',
    configured: status.configured,
    environmentFallbackConfigured: hasAnthropicEnvironmentFallback(),
...
export async function PUT(request: Request) {
  ...
  try {
    const status = await setRuntimeSecret(prisma, {
      name: ANTHROPIC_API_KEY_RUNTIME_SECRET,
...
export async function DELETE(request: Request) {
  ...
  await deleteRuntimeSecret(prisma, ANTHROPIC_API_KEY_RUNTIME_SECRET);
  return noStoreJson({
    provider: 'anthropic',
    configured: false,
    environmentFallbackConfigured: hasAnthropicEnvironmentFallback(),
...
async function parseCredentialBody(
  request: Request,
): Promise<{ value: AnthropicCredentialBody } | { response: NextResponse }> {
  try {
    const bytes = await readRequestBodyBytes(request, MAX_REQUEST_BODY_BYTES);
    const value = JSON.parse(Buffer.from(bytes).toString('utf8')) as unknown;
    if (!isCredentialBody(value)) {
      return { response: noStoreJson({ error: 'Anthropic APIキーの入力が不正です' }, 400) };
    }
    return { value };
  } catch (error) {
...
function isCredentialBody(value: unknown): value is AnthropicCredentialBody {
...
function hasAnthropicEnvironmentFallback(): boolean {
...
function runtimeSecretFailureResponse(error: unknown): NextResponse {
  if (error instanceof RuntimeSecretError) {
    ...
    if (error.code === 'invalid_secret_name' || error.code === 'invalid_secret_value') {
      return noStoreJson({ error: 'Anthropic APIキーの入力が不正です' }, 400);
    }
  }
  return noStoreJson({ error: 'Anthropic APIキーを保存できませんでした' }, 500);
}
