...
export async function DELETE(
  ...
  if (!ROLE_ID_PATTERN.test(roleId) || roleId === guildId || roleId === STUDIO_ROOT_DISCORD_ROLE_ID) {
    return NextResponse.json({ error: '削除対象Roleが不正または保護されています' }, { status: 400 });
  }

  const references = await findConfigurationReferences(guildId, roleId);
  if (references.length > 0) {
    return NextResponse.json(
      { error: 'このRoleはHerta設定から参照されています。先に参照を解除してください。', references },
      { status: 409 },
    );
  }
...
async function findConfigurationReferences(guildId: string, roleId: string): Promise<string[]> {
  ...
  if (containsDiscordRoleReference(settings?.settingsJson, roleId)) references.push('GuildSettings.settingsJson');
  for (const plugin of plugins) {
    if (containsDiscordRoleReference(plugin.config, roleId)) references.push(`Plugin:${plugin.pluginId}`);
  }
