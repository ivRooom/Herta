...
const CREDENTIAL_ENDPOINT = '/api/admin/runtime-secrets/moonshot';

export function MoonshotProviderCredentialSettings() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setStatus(body);
      setApiKey('');
      setMessage('Moonshot APIキーを安全に保存しました');
    } catch {
  ...
  const remove = async () => {
    ...
    const confirmation = status.environmentFallbackConfigured
      ? '保存済みのMoonshot APIキーを削除しますか？ MOONSHOT_API_KEY migration fallbackが構成されています。Runtime Secret Storeが正常な場合はfallbackが使用されるため、この操作だけではAIアクセス停止を保証しません。'
      : '保存済みのMoonshot APIキーを削除しますか？ AI機能は利用できなくなります。';
    ...
    try {
      ...
      setMessage(
        body.environmentFallbackConfigured
          ? 'Console APIキーを削除しました。MOONSHOT_API_KEY migration fallbackが構成されています。'
          : 'Moonshot APIキーを削除しました',
      );
    } catch {
    ...
    <section className="rounded-2xl border border-border bg-surface p-5 shadow-card">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold">Moonshot APIキー</h2>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-muted">
            AI Runtime SettingsでProviderにMoonshotを選んだ場合に使うserver-side
        ...
        <>
          {status.environmentFallbackConfigured ? (
            <div className="mt-5 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs leading-5 text-amber-800 dark:text-amber-200">
              <strong>MOONSHOT_API_KEY migration fallbackが構成されています。</strong>{' '}
              {status.configured
