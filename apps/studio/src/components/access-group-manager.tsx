...
export function AccessGroupManager({
  ...
  async function saveGroup() {
    ...
    try {
      ...
      setNotice({
        kind: 'success',
        text: isNew ? 'Groupを作成しました。' : 'Groupを更新しました。',
      });
      router.refresh();
    } catch (error) {
      setNotice({
        kind: 'error',
        text: error instanceof Error ? error.message : 'Groupの保存に失敗しました。',
      });
    } finally {
      setPending(false);
    }
  ...
  async function deleteGroup() {
    ...
    if (
      !window.confirm(
        `Group「${selectedGroup.name}」を削除しますか？MemberとAttachmentも削除されます。`,
      )
    )
      return;
    setPending(true);
    try {
      ...
      setNotice({
        kind: 'error',
        text: error instanceof Error ? error.message : 'Groupの削除に失敗しました。',
      });
    } finally {
      setPending(false);
    }
  ...
  async function changeMember(targetUserId: string, add: boolean) {
    ...
    try {
      ...
      setNotice({
        kind: 'success',
        text: add ? 'Memberを追加しました。' : 'Memberを削除しました。',
      });
      router.refresh();
    } catch (error) {
      setNotice({
        kind: 'error',
        text: error instanceof Error ? error.message : 'Group Member更新に失敗しました。',
      });
    } finally {
      setPending(false);
    }
            ...
            <div className="rounded-xl border border-border bg-background p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {selectedMembers.length === 0 ? (
                  <span className="text-xs text-muted">Memberなし</span>
                ) : null}
              </div>
            </div>
          ) : null}
