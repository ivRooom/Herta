...
export function ManagedPolicyManager({
  ...
  const directUsers = selectedAttachments.filter((attachment) => attachment.principalType === 'user');
  ...
  function toggleAction(action: StudioPolicyAction) {
    ...
    setDraft(
      JSON.stringify(setStudioGuiActions(parsedDraft ?? emptyPolicy(), guildId, [...next]), null, 2),
    );
    setNotice(null);
  }
  ...
  async function savePolicy() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          [result?.error, ...(result?.details ?? [])].filter(Boolean).join(' / ') || '保存に失敗しました',
        );
      }
      if (isNew && result?.policy?.id) setSelectedPolicyId(result.policy.id);
      setNotice({ kind: 'success', text: isNew ? 'Policyを作成しました。' : 'Policyを更新しました。' });
      router.refresh();
    } catch (error) {
  ...
  async function deletePolicy() {
    ...
    if (!window.confirm(`Policy「${selectedPolicy.name}」を削除しますか？Attachmentも削除されます。`)) return;
    setPending(true);
    try {
  ...
  async function setAttachment(
    ...
    try {
      ...
      setNotice({ kind: 'success', text: attached ? 'PolicyをAttachしました。' : 'PolicyをDetachしました。' });
      if (principalType === 'user' && attached) setUserId('');
      router.refresh();
    } catch (error) {
      setNotice({ kind: 'error', text: error instanceof Error ? error.message : 'Attachment更新に失敗しました。' });
    } finally {
      setPending(false);
    }
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-muted">
            Policyを一度定義し、Discord Role・User・Herta GroupへAttachします。Denyはすべての経路より優先されます。
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
        <aside className="rounded-xl border border-border bg-background p-3">
          <div className="space-y-1" role="list" aria-label="Managed Policy一覧">
            ...
            {policies.length === 0 ? <p className="px-2 py-3 text-sm text-muted">Policyはまだありません。</p> : null}
          </div>
        </aside>
        ...
        <div className="min-w-0 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex rounded-xl border border-border bg-background p-1" role="tablist" aria-label="Policy編集モード">
                <button type="button" role="tab" aria-selected={mode === 'gui'} onClick={() => setMode('gui')} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${mode === 'gui' ? 'bg-primary text-primary-foreground' : ''}`}>GUI</button>
                <button type="button" role="tab" aria-selected={mode === 'json'} onClick={() => setMode('json')} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${mode === 'json' ? 'bg-primary text-primary-foreground' : ''}`}><Braces className="mr-1 inline h-3.5 w-3.5" aria-hidden="true" />JSON</button>
              </div>
            </div>
            {mode === 'gui' ? (
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {STUDIO_POLICY_ACTIONS.map((action) => (
                  <label key={action} className="flex items-center gap-3 rounded-xl border border-border bg-background p-3 text-sm">
                    <input type="checkbox" checked={selectedActions.has(action)} onChange={() => toggleAction(action)} disabled={!canEdit || pending} />
                    <span><span className="font-semibold">{ACTION_LABELS[action]}</span><code className="mt-0.5 block text-[10px] text-muted">{action}</code></span>
                  </label>
                ))}
              </div>
            ) : (
          ...
          {selectedPolicy ? (
            <div className="space-y-4 border-t border-border pt-5">
              ...
              <PrincipalChecklist title="Discord Roles" options={roles} type="role" attachments={selectedAttachments} disabled={!canEdit || pending} onChange={setAttachment} />
              <PrincipalChecklist title="Herta Groups" options={groups} type="group" attachments={selectedAttachments} disabled={!canEdit || pending} onChange={setAttachment} />
              <div className="rounded-xl border border-border bg-background p-4">
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <button type="button" onClick={() => void setAttachment('user', userId.trim(), true)} disabled={!canEdit || pending || !/^\d{17,20}$/u.test(userId.trim())} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm font-semibold disabled:opacity-50"><UserPlus className="h-4 w-4" aria-hidden="true" />Attach</button>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {directUsers.map((attachment) => (
                    <button key={attachment.principalId} type="button" onClick={() => void setAttachment('user', attachment.principalId, false)} disabled={!canEdit || pending} className="rounded-full border border-border px-3 py-1 font-mono text-xs hover:border-red-500/40 hover:text-red-600">{attachment.principalId} ×</button>
                  ))}
                  {directUsers.length === 0 ? <span className="text-xs text-muted">Direct User attachmentなし</span> : null}
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className="flex flex-wrap gap-2 border-t border-border pt-5">
            <button type="button" onClick={() => void savePolicy()} disabled={!canEdit || pending || name.trim().length === 0} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50"><Save className="h-4 w-4" aria-hidden="true" />{selectedPolicy ? '更新' : '作成'}</button>
            {selectedPolicy ? <button type="button" onClick={() => void deletePolicy()} disabled={!canEdit || pending} className="inline-flex items-center gap-2 rounded-xl border border-red-500/30 px-4 py-2 text-sm font-semibold text-red-600 disabled:opacity-50"><Trash2 className="h-4 w-4" aria-hidden="true" />Policy削除</button> : null}
          </div>
        </div>
      </div>
    </section>
...
function PrincipalChecklist({
  ...
  const attachedIds = new Set(
    attachments.filter((attachment) => attachment.principalType === type).map((attachment) => attachment.principalId),
  );
  return (
    <div className="rounded-xl border border-border bg-background p-4">
      ...
      <div className="mt-3 grid max-h-64 gap-2 overflow-y-auto sm:grid-cols-2">
        {options.map((option) => (
          <label key={option.id} className="flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-2 text-sm">
            <input type="checkbox" checked={attachedIds.has(option.id)} onChange={(event) => void onChange(type, option.id, event.target.checked)} disabled={disabled} />
            <span className="min-w-0"><span className="block truncate font-medium">{option.name}</span><code className="block truncate text-[10px] text-muted">{option.id}</code></span>
          </label>
        ))}
...
function extractGuiActions(policy: StudioAccessPolicy | null): Set<StudioPolicyAction> {
  ...
  return new Set(guiStatement.Action.filter((action): action is StudioPolicyAction => supported.has(action)));
}
