...
test('Plugin Operations inventory keeps separate mobile cards and desktop table views', () => {
  ...
  assert.match(operationsPageSource, /<table className="w-full min-w-\[960px\] text-left text-sm">/u);
});
...
test('Plugin Operations mobile cards expose the same safe inventory metadata', () => {
  ...
  assert.match(operationsPageSource, /<RuntimeConsumerBadges states=\{entry\.runtimeConsumers\} \/>/u);
