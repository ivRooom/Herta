...
test('readable Runtime Secret makes Moonshot available without using env fallback', async () => {
  let reads = 0;
  const result = await resolveMoonshotProviderCredentialAvailability({
    readRuntimeCredential: async () => {
      reads += 1;
      return 'runtime-secret-value';
    },
...
test('missing Moonshot Runtime Secret can use the legacy environment fallback', async () => {
  const result = await resolveMoonshotProviderCredentialAvailability({
    readRuntimeCredential: async () => null,
    environmentCredential: ' environment-secret-value ',
  });
...
test('Moonshot Runtime Secret Store failure stays fail closed even when env fallback exists', async () => {
  const result = await resolveMoonshotProviderCredentialAvailability({
    readRuntimeCredential: async () => {
      throw new Error('store unavailable');
    },
...
test('Moonshot is unavailable when neither credential source is usable', async () => {
  const result = await resolveMoonshotProviderCredentialAvailability({
    readRuntimeCredential: async () => null,
    environmentCredential: '   ',
  });
